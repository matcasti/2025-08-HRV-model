...
**Supplementary Figure S3: Full Posterior Recovery of Ground-Truth Parameters.** Posterior density plots for all model parameters across the three simulation scenarios (A, B, C). Vertical black lines indicate the true generative values.
...
We generated synthetic data using a Student's-t distributed noise term (with 3 degrees of freedom) instead of a Gaussian one. Supplementary Figure S4 shows that while the model still recovers the primary trajectories, the estimate for the structured variance fraction ($w$) is biased downwards, meanwhile the SDNN magnitude parameters ($\alpha_s,\beta_s$) compensate the added noise by scaling up in response. The model incorrectly attributes the heavy-tailed noise to the unstructured residual component, which is expected given the model design to model residual variance coming from a Normal distribution.

![](figures/fig-student-t.svg){width=100%}

**Supplementary Figure S4: Impact of Non-Gaussian Noise.** In the upper panel, the estimated signal recovered from posterior predictions (in red) overlaid the generated synthetic signal from a heavy-tailed Student's-t noise (in grey). Lower panel shows the posterior distribution for all model parameters when the model is fit to data with heavy-tailed Student's-t noise. The ground truth parameters are well recovered except for structured variance fraction $w$ and total signal variability magnitude parameters $\alpha_s,\beta_s$.
