...
![](figures/fig-ppcheck.jpeg){width=100%}
...
![](figures/fig-prior-sensitivity.jpeg){width=100%}
...
![](figures/fig-full-recovery.jpeg){width=100%}
...
![](figures/fig-asymetric.jpeg){width=100%}
...
![](figures/fig-student-t.jpeg){width=100%}
...
![](figures/fig-poc-traceplot.jpeg){width=100%}
...
![](figures/fig-poc-posterior.jpeg){width=100%}
...
![](figures/fig-poc-ppcheck.jpeg){width=100%}
...
![](figures/fig-poc-stat-computed.jpeg){width=100%}
...
![](figures/fig-window-sensitivity.jpeg){width=100%}
...
As a further, more robust spectral comparator, we implemented a continuous wavelet transform (Morlet wavelet) analysis, given that the CWT does not impose a single, fixed time-frequency resolution trade-off across all frequencies, unlike the STFT. Band-wise power proportions (VLF, LF, HF) were extracted from the resulting scalogram by integrating power within each canonical band at each time step and normalizing to yield proportional power, using the same per-time-step normalization applied to the STFT output (Section 4.3.2 of the main text); this is directly comparable to the model's implied variance-share quantity, $q_j(t_i)$ (Section 4.1.3 of the main text), rather than to the raw mixture weight $p_j(t_i)$. *[Results placeholder: CWT implementation code and reconstruction results, pending completion of the corresponding analysis.]*


## S4.3: Comparator Uncertainty and Computational Cost

Unlike the generative model, the windowed, STFT, and CWT comparators are deterministic given the data and therefore have no native posterior uncertainty. To report comparable uncertainty across all methods in Table 3, we computed 95% bootstrap intervals for each comparator by resampling the underlying synthetic record and recomputing the relevant error metric. *[Results placeholder: bootstrap procedure code and resulting intervals, pending completion of the corresponding analysis.]* Wall-clock runtime for the generative model (HMC sampling) and each comparator was recorded on the same hardware (Section S1.3) to contextualize the trade-off between reconstruction fidelity and computational cost. *[Results placeholder: runtime comparison table, pending completion of the corresponding benchmarking.]*
