...
::: {custom-style="MDPI_4.1_table_caption"}
**Table 3. Quantitative Performance Metrics for the Generative Model versus Windowed and Wavelet-Based Methods.** This table presents the root mean squared error (RMSE) and mean absolute error (MAE) as measures of the average magnitude of the estimation error; Bias (mean error) identify any systematic tendency for over- or underestimation; and the coefficient of determination ($R^2$) to assess the proportion of variance in the true signal captured by the estimate. *[Pending: Continuous Wavelet Transform (CWT) comparator rows to be added for the spectral-proportion metrics.]*
:::
...
We partition the total signal variability, $\mathrm{SDNN}(t_i)^2$, into two components. This partition is governed by a single parameter, $w \in [0,1]$, which represents the fraction of total variance that is considered physiologically structured (i.e., oscillatory). The remaining fraction, ($1 - w$), is treated as unstructured residual noise. This relationship is formalized in in @eq-var-structure.
...
Here, $\alpha_s$, $\beta_s$, and $c_s$ are analogous to their counterparts in the baseline model, representing the resting total SDNN, the magnitude of its suppression or increase, and its fractional recovery, respectively. The $\mathrm{SDNN}(t_i)$ trajectory thus serves as a master controller for the total instantaneous variability in the signal. This total variance is then partitioned into a structured, oscillatory component and an unstructured, residual noise component, as defined in @eq-var-structure.

The dynamics for both trajectories (i.e., $\mathrm{RR_{base}}$ and $\mathrm{SDNN}$), are driven by a shared pair of logistic transition functions, $\mathcal{D}_{1}(t_i)$ and $\mathcal{D}_{2}(t_i)$, defined in @eq-logistic-components.
...
**Data Availability Statement**: The Stan code implementing the complete generative model is available as a supplementary material. The raw data supporting the conclusions of this article will be made available by the authors without undue reservation.
