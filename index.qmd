...
$$
\vec{v}_j \sim \mathcal{GP}(\vec{0}, \mathbf{K}_j), \quad \text{where } (\mathbf{K}_j)_{kl} = \exp\left(-\frac{(\tilde{f}_{j,k} - \tilde{f}_{j,l})^2}{2\,\rho_{gp,j}^2}\right)
$${#eq-gp-kernel}
