...
We implement this generative approach within a hierarchical Bayesian framework, which is essential for three reasons. First, it allows for the robust estimation of parameters and their associated uncertainty, treating inference as a problem of probability rather than optimization. Second, its hierarchical structure enables the simultaneous modeling of individual- and group-level effects, capturing both unique response profiles and shared population characteristics. Third, it provides a principled basis for model comparison, allowing researchers to formally adjudicate between competing hypotheses about the underlying process. This paper tests the central hypothesis that this generative framework provides a more accurate, precise, and scientifically useful characterization of autonomic dynamics than is possible with existing methods. Through simulation and application to empirical data, we demonstrate that this approach can recover the ground-truth parameters of a known process and provides novel insights into human stress physiology. We contend that this methodology represents a necessary step forward, providing an extensible framework for translating complex physiological time-series into falsifiable, mechanistic knowledge.
