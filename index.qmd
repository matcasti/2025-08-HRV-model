...
#| message: false
#| warning: false
readRDS("tables/tbl-3A.RDS")
```

::: {custom-style="MDPI_4.1_table_caption"}
**Table 3B. Quantitative Performance Metrics for the Generative Model across Simulated Conditions (A, B and C).** This table presents the root mean squared error (RMSE) and mean absolute error (MAE) and SDNN-scale metrics, and as a dimensionless proportion for spectral (implied variance-share, $q_j(t)$) metrics, as measures of the average magnitude of the estimation error; Bias (mean error, same units) to identify any systematic tendency for over- or underestimation; and the coefficient of determination ($R^2$, dimensionless) to assess the proportion of variance in the true signal captured by the estimate. Intervals reported for the generative model are 95% posterior credible intervals computed across MCMC draws.
:::
...
#| message: false
#| warning: false
readRDS("tables/tbl-3B.RDS")
```


::: {custom-style="MDPI_3.1_text"}
Further model diagnostics testing the model's overall stability and reliability on synthetic data for *in silico* validation can be found in the supplementary material. Prior predictive checks are shown in Figure S1, prior sensitivity analysis in Figure S2, full posterior distributions in Figure S3, and model fit to asymmetric recovery data from a different data generation process in Figure S4. Additionally, non-Gaussian distributions are presented in Figure S5. Beyond these three fixed scenarios, we also report a pairwise posterior-correlation analysis for $\beta$, $c$, $\delta$, and $\phi$, and replicate the asymmetric-recovery and non-Gaussian misspecification checks quantitatively across repeated simulation runs rather than as single visual examples. We further strengthened the residual diagnostics for all misspecification checks to include autocorrelation, conditional-variance, and tail-behavior assessments, rather than global mean/SD recovery alone. Consistent with the model's stated scope as a single-event instrument (Section 3.3), we did not extend these checks to multiple or overlapping perturbations.
:::
...
This proposed dissociation is not without physiological precedent, and situating it against known autonomic kinetics helps clarify why it is a plausible hypothesis rather than an ad hoc pattern. The two branches of the autonomic nervous system act on markedly different timescales: parasympathetic (vagal) influence on the sinoatrial node is mediated by acetylcholine acting through fast-activating potassium channels and is rapidly terminated by local acetylcholinesterase activity, producing beat-to-beat changes in heart period within a few cardiac cycles [@maltsev2023novel; @varzideh2026autonomic]. Sympathetic influence, by contrast, is mediated by norepinephrine acting through slower second-messenger cascades and is cleared more gradually through reuptake and diffusion, so its effects unfold over tens of seconds to minutes [@maltsev2023novel; @varzideh2026autonomic]. If the comparatively fast recovery of mean RRi observed here reflects predominantly vagal reactivation, while the slower recovery of total variability reflects a more gradual sympathovagal rebalancing, a dissonant pattern of this kind is broadly consistent with these established differences in autonomic kinetics.
