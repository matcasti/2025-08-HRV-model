...
Bridging the gap between data and mechanism requires an evolution in analytical philosophy, a shift from model-agnostic data description to the construction of hypothesis-driven, generative models. Such approach should move beyond simply characterizing what the data looks like and instead focuses on creating explicit, testable, and quantitative models of the biological processes that produced it. Here, we introduce and validate such a framework, which reframe physiological analysis as a form of formal hypothesis testing. We posit that a stereotyped autonomic response to an acute perturbation is a structured, dynamic process. Consequently, we propose it can be effectively modeled as a continuous-time stochastic process, allowing us to separate the structured response from inherent biological variability. As a specific, testable implementation of this philosophy, we model the response trajectory using a double-logistic function. This mathematical form is chosen as a parsimonious hypothesis for biphasic dynamics, designed to capture the characteristic onset and subsequent recovery phases [@castillo2025enhancing]. The power of this approach lies in its ability to translate the raw signal into a small set of interpretable mechanistic parameters: the latency, rate, and magnitude of each distinct response phase [@castillo2025enhancing]. This provides a direct, quantitative window into the system's underlying reactivity and resilience.

We implement this approach as a novel generative model within a Bayesian framework. This provides robust parameter estimation with full uncertainty quantification, enables hierarchical modeling of individual and group-level effects, and offers a basis for model comparison. This model provides a specific and testable implementation of our framework, decomposing the RRi series into a time-varying mean, total variance, and a dynamically shifting oscillatory component, all controlled by a double-logistic latent dynamic. This paper tests the hypothesis that this model provides a more accurate and useful characterization of autonomic dynamics than existing fixed-time window methods. Through simulation, we demonstrate its capacity to recover known ground-truth parameters, which establishes its core validity. We contend that this approach provides the necessary quantitative indices to develop and test mechanistic physiological hypotheses. This represents a step towards translating complex time-series into falsifiable knowledge.
...
A central component of the model is the partitioning of the total signal variance into two distinct, time-dependent components: a structured variance, $\sigma^2_{\text{struct}}(t_i)$, which captures the physiologically meaningful, oscillatory patterns of HRV, and a residual variance, $\sigma^2_{\text{resid}}(t_i)$, which accounts for unstructured, moment-to-moment fluctuations. However, physiological recordings are frequently contaminated by artifacts or ectopic beats that are not well-described by a simple Gaussian noise process. To ensure the model is robust to such outliers, we depart from a unimodal likelihood. Instead, we specify the observation model as a two-component finite mixture model, which conceptualizes each data point as arising from one of two states: a primary "inlier" state, or a rare "outlier" state.

The inlier state assumes the data follows the core physiological model, with residuals distributed normally. The outlier state is captured by a heavy-tailed Cauchy distribution, which can account for extreme, aberrant observations without influencing the estimation of the core dynamic parameters. The complete likelihood is a mixture of these two distributions, as defined in @eq-full-likelihood.

$$
\begin{aligned}
z_i &\sim \text{Bernoulli}(p_{\text{out}}) \\
\mathrm{RRi}(t_i) | z_i=0 &\sim \mathcal{N}\left(\mu(t_i), \sigma_{\text{resid}}(t_i)\right) \\
\mathrm{RRi}(t_i) | z_i=1 &\sim \text{Cauchy}\left(\mu(t_i), \sigma_{\text{resid}}(t_i)\right)
\end{aligned}
$${#eq-full-likelihood}

Here, a latent binary variable $z_i$ determines if an observation is an inlier ($z_i=0$) or an outlier ($z_i=1$) with probability $p_{\text{out}}$. Inliers follow a Normal distribution centered on the model's dynamic prediction $\mu(t_i)$, while outliers follow a heavy-tailed Cauchy distribution, sharing the scale $\sigma_{\text{resid}}(t_i)$.
...
The covariance matrix $\mathbf{K}_j$ is influenced by two hyperparameters which are themselves estimated from the data: $\alpha_{gp,j}$, which correspond to the marginal standard deviation, which controls the overall magnitude or "waviness" of the spectral shape. A larger $\alpha_{gp,j}$ allows for larger peaks in the spectrum. Moreover, $\rho_{gp,j}$ is the length-scale, which controls the smoothness of the function. A large $\rho_{gp,j}$ enforces a very smooth, slowly changing shape, while a small value allows for more rapid variations and narrower spectral peaks.
...
This inversion dynamically adjusts the amplitude of the synthesized spectral signal to ensure its contribution to the total variance is exactly as prescribed by the model's parameters ($\mathrm{SDNN}(t_i)$ and $w$), thereby connecting the time-domain and frequency-domain components of the model.
...
The positive rate parameters $\lambda$ and $\phi$ are simply log-transformed, i.e., $\lambda = \exp(\lambda_\text{log})$. The recovery coefficients for the time-domain components ($c_r,c_s$) are mapped to the interval $[0, 2]$, while the recovery parameter for spectral components ($c_c$) is mapped to the $[0, 1]$ interval, using a scaled logit function. Similarly, the fraction of structured variance, $w$, is constrained between 0 and 1 via $w = \text{logit}^{-1}(w_{\text{logit}})$. The probability of an observation being an outlier, $p_{\text{out}}$, is likewise mapped from the logit scale, $p_{\text{out}} = \text{logit}^{-1}(p_{\text{out,logit}})$. We place a weakly informative prior on this parameter, $p_{\text{out,logit}} \sim \mathcal{N}(\text{logit}(0.01), 0.5)$, which centers our prior belief on a 1% outlier rate but allows the data to update this belief.
