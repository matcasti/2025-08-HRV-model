...
Heart rate variability (HRV) is a cornerstone of non-invasive autonomic nervous system assessment, yet its analysis is hampered by the non-stationary nature of physiological signals. Standard analytical methods, which assume stationarity within fixed time windows, fundamentally distort the underlying dynamics of interest, such as the response to a physiological stressor. This limitation obstructs the development of mechanistic hypotheses about autonomic control. Here, we address this challenge by introducing a unified probabilistic framework for non-stationary HRV analysis. We propose a hypothesis-driven, generative model that recasts the physiological response as a continuous-time stochastic process governed by a double-logistic function. This approach deconstructs the R-R interval (RRi) series into a set of interpretable parameters representing the latency, rate, and magnitude of distinct response and recovery phases. Through simulation, we demonstrate the model's ability to achieve high-fidelity parameter recovery and show its descriptive superiority over conventional windowed methods. We then apply the framework to an empirical exercise-recovery recording, generating a precise, falsifiable hypothesis of "dissonant autonomic recovery", where mean heart rate and its variability exhibit distinct recovery timescales. This framework provides a formal methodology for translating complex biosignals into quantitative, testable estimates of their generative processes.
